...
//		return false;
	...
	public int insert(Bmember DTO) {
		...
		int result = 0;
		try {
			result = session.insert("memberJoin", DTO);
		} finally {
			session.close();
		}
		return result;
...
//		return 0;
	...
	public String selectlogin(String id, String pw) {
		...
		try {
			...
			hashmap.put("id", id);
			hashmap.put("pw", pw);
			System.out.println("dto : " + hashmap.get("id") + hashmap.get("pw"));
			result = session.selectOne("loginCheck", hashmap);
		} finally {
			session.close();
		}
		return result;
...
//		return hm;
	 ...
	 */
	public ArrayList<Bmember> selectId(String name) {
		ArrayList<Bmember> al = new ArrayList<>();
